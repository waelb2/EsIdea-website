...
const Card = ({proj,index,openedMore,setOpenedMore}) => {
  const {setIdProjects,setEditProjectPopUp,setCollaborators,setCollaboratorPopUp} = useContext(projectContext);
  const projectDetails = [
    ...
    },
    {
        ...
        line:true,
        action:()=>{
          setIdProjects(proj.ProjectTitle);
  ...
  return (
    <div className={`group flex flex-col w-full  rounded-xl   relative transition-all duration-500   hover:shadow-lg borderGrey cursor-pointer`}>
        ...
        <div onClick={(e)=>{e.stopPropagation()}}>
          ...
          <div className={`${index !== openedMore ?"hidden":"bg-realWhite min-w-[10.625rem] absolute shadow-2xl bottom-8 right-4 border p-3 rounded-lg border-black sidebar"}`}>
              <ul>
