...
const CreateNewProject = ({
  ...
  const createProject = async () => {
    ...
    axios
      .post('project/create-project', myForm, {
        headers: {
          Authorization: `Bearer ${userToken}`,
  ...
  return (
    <div
      onClick={closePopUp}
      className={`fixed top-0 left-0 bg-black bg-opacity-40 w-screen min-h-screen backdrop-blur z-50 duration-500  transition-opacity ease-in-out flex justify-center items-center  ${
        visible ? 'Pop-up-Active' : 'Pop-up-notActive'
      }`}
    >
      <div
        onClick={e => {
        ...
        className='bg-white max-w-full w-[37.5rem]   rounded-2xl shadow-md  px-3 py-4 sm:py-7 sm:px-9 m-4'
      >
        <div className={`flex flex-col justify-center items-center`}>
          ...
          <div
            className={`w-full ${
              currentPage === 1 ? 'flex flex-col items-center' : 'hidden'
            }`}
          >
            ...
            <div className='flex w-full  self-center border-2 rounded-md ss:p-1 shadow-md mb-5 p-2'>
              <input
                ...
                className='flex-grow flex-shrink w-full outline-none ml-2 text-sm'
                placeholder='Search'
                type='text'
              />
              <img
                className='w-5 h-5 ss:w-7 ss:h-7 ml-1'
                src={Search}
                alt='Search'
              />
            </div>
            ...
            <div className='flex flex-wrap gap-3 justify-center  ss:px-9 ss:justify-start overflow-y-auto scroll-smooth py-2'>
              {displayedMethods.map(method => (
                <div
                  ...
                  className='bg-lightGrey w-[9rem] h-[7rem] rounded-md flex flex-col items-center justify-center cursor-pointer border-2 border-transparent transition-all duration-500 hover:border-slate-400'
                  key={method.Id}
                >
                  <img
                    className='w-16 h-16'
          ...
          <div
            className={`w-full ss:px-3 ${
              currentPage === 2 ? 'flex flex-col items-center' : 'hidden'
            }`}
          >
            <h1 className='font-bold text-2xl text-center mb-4'>
            ...
            <div className='flex flex-col w-full  gap-1 mb-3'>
              ...
              <input
                ...
                id='projectName'
                className='outline-none text-sm border border-grey rounded-md py-2 px-4'
                type='text'
              />
            </div>
            ...
            <div className='flex flex-col w-full  gap-1 mb-3'>
              ...
              <textarea
                ...
                className='outline-none text-sm border border-grey rounded-md resize-none py-2 px-4'
                name='Description'
            ...
            <div className='flex flex-col w-full gap-1 mb-3'>
              <label
                htmlFor='projectTimer'
                className='text-black font-semibold'
              >
                Set project timer (in seconds)
              </label>
              <input
                onChange={e => setProjectTimer(e.target.value)}
                value={projectTimer}
                id='projectTimer'
                className='outline-none text-sm border border-grey rounded-md py-2 px-4'
                type='number'
            ...
            <div className='flex flex-col w-full  gap-1 mb-3'>
              <h1 className='text-black font-semibold'>
                In which category do you wanna put your project{' '}
              </h1>
              <div className='border border-grey rounded-md py-3 px-4 mb-2 '>
                {tagsDisplayed.length !== 0 ? (
                  <div className='w-full flex flex-wrap gap-2 max-h-16 overflow-y-auto'>
                    {tagsDisplayed.map((val, ind) => (
                      <div
                        className='flex gap-1 p-1 items-center rounded-md bg-gray-200'
                        key={ind}
                      >
                  ...
                  <p className='text-xs text-gray-700'>
                    Please use the lists bellow to associate a category to your
                    project
                  </p>
                )}
              </div>
              <DependenciesContext.Provider value={dependencies}>
                <div
                  ...
                  className='flex gap-2 items-center flex-wrap relative'
                >
                  <Select
                    title={'module'}
                    data={modules}
                    accesorKey={'moduleName'}
                  />
                  <Select title={'club'} data={clubs} accesorKey={'clubName'} />
                  <Select
                    title={'event'}
                    data={events}
                    accesorKey={'eventName'}
                  />
                </div>
          ...
          <div
            className={`w-full  ${
              currentPage === 3 ? 'flex flex-col items-center' : 'hidden'
            }`}
          >
            <h1 className='font-bold text-2xl text-center mb-4'>
            ...
            <label
              className='text-black font-semibold self-start mb-2'
              htmlFor='collaborator'
            >
              Do you want to add some collaborator
            </label>
            ...
            <div className='w-full  self-center relative'>
              <div className='flex w-full  self-center border-2 rounded-md ss:p-1 shadow-lg mb-5 p-2'>
                <input
                  value={searchColl}
                  onChange={e => {
                    setSearchColl(e.target.value)
                  }}
                  className='flex-grow w-full flex-shrink outline-none ml-2 text-sm'
                  id='collaborator'
                  placeholder='Search'
                  type='text'
                />
              ...
              <div
                className={`border shadow-md absolute top-10 rounded-md p-1 w-full mb-3 bg-realWhite ${
                  debouncedSearch !== '' || loading ? 'block' : 'hidden'
                }`}
              >
                {loading ? (
                  ...
                  debouncedSearch !== '' ? (
                  <div className='flex flex-col gap-1 max-h-44 overflow-y-auto scroll-smooth'>
                    {displayedCollaborators.map(collaborator => (
                      <div
                        className='flex justify-between items-center border rounded-md border-black p-2'
                      ...
                      >
                        ...
                        <button
                          ...
                          className='bg-skyBlue text-realWhite px-2 py-0.5 rounded-lg'
                        >
                          invite
                        </button>
                      </div>
            ...
            <div className='w-full  self-center mb-2'>
              {collaborators.length === 0 ? (
                <div className='flex flex-col items-center justify-center max-h-64'>
                  <img className='w-32 h-32' src={ProjectsEmpty} alt='empty' />
                  <p className='text-base'>{'No collaborator :('}</p>
                  <p className='text-center text-base'>
                    Don’t worry you can add them by just searching there names.
                  </p>
                </div>
              ) : (
                <div className='flex flex-col gap-1 overflow-y-auto scroll-smooth max-h-56'>
                  {collaborators.map((collaborator, ind) => (
                    <div
                      className='flex justify-between items-center border rounded-md border-black p-1 w-full'
                    ...
                    >
                      ...
                      <button
                        onClick={() => {
                          handleRemoveCollaborator(ind)
                        }}
                        className='bg-red text-realWhite px-2 py-0.5 rounded-lg'
                      >
                        Remove
                      </button>
                    </div>
          ...
          <div
            className={`w-full ss:px-3 ${
              currentPage === 4 ? 'flex flex-col items-center' : 'hidden'
            }`}
          >
            <h1 className='font-bold text-2xl text-center mb-4'>
              New <span className='text-skyBlue'>{method}</span>
            ...
            <div className='flex flex-col w-full gap-1 mb-3'>
              <div className='flex justify-between items-center'>
                <h1 className='text-black font-semibold'>
                  Do you want to add sub topics
                </h1>
                <button
                  onClick={handleAddNewSubTopic}
                  className='bg-skyBlue py-1 px-2 rounded-lg text-realWhite text-nowrap'
                >
                  Add new +
                </button>
              </div>
              <div
                className={`${
                  addSubTopicState ? 'block' : 'hidden'
                } flex justify-between items-center border rounded-md border-gray-700 p-2 gap-2`}
              >
                <input
                  autoFocus
                  ...
                  onChange={e => {
                    setSubTopicInputValue(e.target.value)
                  }}
                  className='text-sm py-1 px-2 border border-gray-400 outline-none flex-grow rounded-sm'
                  placeholder='Add sub topic here'
                  type='text'
                />
                <button
              ...
              <div>
                {subTopics.length === 0 ? (
                  <div className='flex flex-col items-center justify-center'>
                    ...
                    <p className='text-center text-base'>
                      Don’t worry you can make a section with no sub topics and
                      you can also add new ones her.
                    </p>
                  </div>
                ) : (
          ...
          <div
            className={`w-full ss:px-3  ${
              currentPage === 5 ? 'flex flex-col items-center' : 'hidden'
            }`}
          >
            <h1 className='font-bold text-2xl text-center mb-4'>
              New <span className='text-skyBlue'>{method}</span>
            ...
            <div className='my-2 w-full'>
              <h1 className='text-black font-bold mb-2'>
                Upload a picture to use it as a banner for the project ( you can
                not use one )
              </h1>
              {!banner ? (
                <div
                  ...
                  className='flex flex-col items-center justify-center gap-1 border-2 border-dashed border-skyBlue rounded-lg p-2'
                >
                  <img src={dragdropfiles} alt='files' />
                  <h1 className='text-black font-bold'>
                    Drag and Drop pic to upload
                  </h1>
                  ...
                  <button
                    ...
                    className='bg-skyBlue py-1 px-2 rounded-xl text-realWhite'
                  >
                    Browse
                  </button>
                </div>
              ) : (
                <div className='flex flex-col gap-4 border-2 border-gray-600 rounded-md p-2 justify-center items-center flex-grow'>
                  <img
                    className='w-72 object-contain rounded-md'
                    src={banner}
                    alt='banner'
                  />
                  <button
                    ...
                    className='bg-red text-realWhite py-1 px-2 rounded-lg'
                  >
                    Cancel
                  </button>
                </div>
          ...
          <div
            className={`w-full ss:px-3 self-end  ${
          ...
          >
            <button
              onClick={handleNext}
              className={`bg-skyBlue px-3 py-1 rounded-md text-white ${
                currentPage < 5 ? 'inline' : 'hidden'
              }`}
            >
              Next
            </button>
            <button
              ...
              className={`bg-skyBlue text-center px-3 py-1 rounded-md text-white ${
                currentPage < 5 ? 'hidden' : 'inline'
              }`}
            >
              {createProjectState ? (
                <ThreeDots
