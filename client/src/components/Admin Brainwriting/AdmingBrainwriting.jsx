...
const AdminBrainWriting = ({ project, ideas, onlineUsers, socket }) => {
  const initialMinutes = Math.floor(project.timer / 60)
  const initialSeconds = project.timer % 60
  ...
  useEffect(() => {
    let collaborators = []
    collaborators.push({
      lastName: project.coordinator.firstName,
      imageUrl: project.coordinator.profilePicUrl
    })
      ...
      <div className='flex flex-col items-end absolute right-0'>
        {userArray.map((user, index) => (
          <User
            ...
            useGradientBorder={index === activeUserIndex}
            showName={index === activeUserIndex}
          />
        ))}
      </div>
        ...
        >
          ...
          {showExtendPopUp && (
            <Extend
              ...
              updateUserThoughts={setUserThoughts}
            />
          )}
        </div>
      )}
